...
export function buildPhaseCUserPrompt(intakeContext, phaseAOutput, phaseBOutput, metadata) {
  ...
  const compactNote = isLargeOrg
    ? `\n\nCOMPACT MODE (${metadata.planCount} plans): Keep rationales to 1-2 sentences each. Keep scenarios to 3. Keep operational sections concise.`
    : ''
...
3. SCENARIOS: Generate ${isLargeOrg ? '3' : '3-5'} what-if scenarios. Each must:
