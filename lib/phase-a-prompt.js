...
export function buildPhaseAUserPrompt(intakeContext, metadata) {
  ...
  const compactNote = pc > 12
    ? `\n\nMAX COMPACT MODE (${pc} plans): You MUST include all ${pc} roles in role_architecture. CRITICAL token efficiency rules:
- Each role direction field: ONE short sentence max.
- For roles sharing the same base approach across segments (e.g., all segment AEs), write directions for the FIRST one fully, then use "Same as [role_key] except: [differences]" for the rest.
- strategic_analysis sections: 1-2 sentences each.
- key_design_decisions: 3 max.
- anti_patterns_detected: top 2-3 only.
- behavioral_prediction and key_risks: skip for roles using "Same as" references.`
    : pc > 5
    ? `\n\nCOMPACT MODE (${pc} plans): Keep each role's direction fields to ONE sentence each. Keep strategic_analysis sections brief (2-3 sentences, not paragraphs). Keep key_design_decisions to 3-4 max. Keep anti_patterns_detected to the most critical only. You MUST include all ${pc} roles in role_architecture but be concise per role. For roles that share the same base approach (e.g., all segment AEs with same structure), you can note "Same approach as [role_key]" in direction fields and only detail what differs.`
    : ''
