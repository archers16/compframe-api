...
/**
 * Translate Phase 1 simplified intake fields to the field names the pipeline expects.
 * Detects new intake by presence of 'funding_stage' (Phase 1) vs 'company_stage' (legacy).
 * Returns a new intake object with translated field names; original is not mutated.
 */
function translatePhase1Intake(intake) {
  // If the intake already has company_stage, it's the old format; no translation needed
  if (intake.company_stage) return intake

  // If no funding_stage either, return as-is (empty or unknown format)
  if (!intake.funding_stage) return intake

  const translated = { ...intake }

  // Company profile
  const stageMap = { bootstrapped: 'seed', pre_seed_seed: 'seed', series_a: 'series_a', series_b: 'series_b', series_c_plus: 'growth' }
  translated.company_stage = stageMap[intake.funding_stage] || 'series_a'

  // Team size: map new ranges to old ranges
  const teamMap = { '0': '1-5', '1-3': '1-5', '4-10': '6-15', '11-20': '16-30', '21-30': '31-75' }
  translated.team_size = teamMap[intake.team_size] || intake.team_size || '1-5'

  // Hiring plan: map new ranges to old ranges
  const hiringMap = { '0': '0', '1-3': '1-5', '4-10': '6-15', '10+': '16-30' }
  translated.hiring_plan = hiringMap[intake.hiring_plan] || intake.hiring_plan || '0'

  // Product type -> deal_structure
  const productMap = { saas: 'recurring', services: 'one_time', hardware: 'one_time', marketplace: 'consumption', other: 'mixed' }
  if (intake.product_type && !intake.deal_structure) {
    translated.deal_structure = productMap[intake.product_type] || 'recurring'
  }

  // Industry
  if (intake.industry && !intake.industry_vertical) {
    translated.industry_vertical = intake.industry
  }

  // Buying motion -> sales_motion
  if (intake.buying_motion && !intake.sales_motion) {
    translated.sales_motion = intake.buying_motion
  }

  // Revenue focus -> revenue_mix
  const revenueMap = { all_new: 'all_new', mostly_new: 'mostly_new', balanced: 'balanced', mostly_expansion: 'mostly_expand' }
  if (intake.revenue_focus && !intake.revenue_mix) {
    translated.revenue_mix = revenueMap[intake.revenue_focus] || 'balanced'
  }

  // Deal involvement -> selling_model
  const dealInvMap = { solo: 'solo', multi: 'team_light', varies: 'team_light' }
  if (intake.deal_involvement && !intake.selling_model) {
    translated.selling_model = dealInvMap[intake.deal_involvement] || 'solo'
  }

  // Lead distribution -> territory_model
  const leadMap = { assigned: 'named', round_robin: 'round_robin', self_sourced: 'greenfield', mix: 'hybrid_territory' }
  if (intake.lead_distribution && !intake.territory_model) {
    translated.territory_model = leadMap[intake.lead_distribution] || 'round_robin'
  }

  // Contract structure: use for clawback context
  if (intake.contract_structure && !intake.deal_structure) {
    const contractMap = { monthly: 'recurring', annual: 'recurring', multi_year: 'recurring', mix: 'mixed' }
    translated.deal_structure = contractMap[intake.contract_structure] || 'recurring'
  }

  // Quota maturity -> quota_confidence
  const quotaMap = { none: '1', informal: '2', formal_miss: '3', formal_ok: '4' }
  if (intake.quota_maturity && !intake.quota_confidence) {
    translated.quota_confidence = quotaMap[intake.quota_maturity] || '2'
  }

  // Plan status: first_plan -> net_new, redesign -> redesign_broken
  const statusMap = { first_plan: 'net_new', redesign: 'redesign_broken' }
  if (intake.plan_status) {
    translated.plan_status = statusMap[intake.plan_status] || intake.plan_status
  }

  // Current problem -> plan_pain_points
  const problemMap = {
    not_motivated: ['wrong_behaviors'], top_leaving: ['top_performers_leaving'],
    too_expensive: ['market_misalignment'], too_complex: ['too_complex'],
    gaming: ['gaming'], not_sure: ['unclear_earnings']
  }
  if (intake.current_problem && !intake.plan_pain_points) {
    translated.plan_pain_points = problemMap[intake.current_problem] || []
  }

  // Primary goal -> leadership_priorities
  const goalMap = {
    drive_revenue: ['growth'], retain_reps: ['talent'], control_costs: ['profitability'],
    align_team: ['growth', 'profitability'], replace_broken: ['growth']
  }
  if (intake.primary_goal && !intake.leadership_priorities) {
    translated.leadership_priorities = goalMap[intake.primary_goal] || ['growth']
  }

  // Admin appetite -> comp_admin_owner
  const adminMap = { minimal: 'spreadsheet_owner', moderate: 'crm_basic', dedicated: 'comp_tool' }
  if (intake.admin_appetite && !intake.comp_admin_owner) {
    translated.comp_admin_owner = adminMap[intake.admin_appetite] || 'spreadsheet_owner'
  }

  // Timeline -> rollout_timeline
  const timelineMap = { immediately: 'asap', within_30: 'next_quarter', within_90: 'next_quarter', planning: 'exploring' }
  if (intake.timeline && !intake.rollout_timeline) {
    translated.rollout_timeline = timelineMap[intake.timeline] || 'next_quarter'
  }

  // Clawback preference: store for prompt context
  if (intake.clawback_preference) {
    translated._clawback_preference = intake.clawback_preference
  }

  // OTE range: store for prompt context
  if (intake.ote_range) {
    translated._ote_range_hint = intake.ote_range
  }

  // Miss handling: store for prompt context
  if (intake.miss_handling) {
    translated._miss_handling = intake.miss_handling
  }

  // Pricing discretion: map to rep_control
  const pricingMap = { fixed: 'low', some: 'medium', significant: 'high' }
  if (intake.pricing_discretion && !intake.rep_control) {
    translated.rep_control = pricingMap[intake.pricing_discretion] || 'medium'
  }

  // Set early-stage defaults for fields that won't exist in Phase 1 intake
  if (!translated.data_maturity) translated.data_maturity = 'low'
  if (!translated.has_segments) translated.has_segments = 'single'
  if (!translated.geographic_spread) translated.geographic_spread = 'us_distributed'
  if (!translated.enablement_maturity) translated.enablement_maturity = 'early'

  // Mark as Phase 1 for any downstream code that needs to know
  translated._phase1_intake = true

  console.log('[Pipeline] Phase 1 intake detected, translated fields:', Object.keys(intake).filter(k => !k.startsWith('_')).join(', '))

  return translated
}

export function buildUserPrompt(intake) {
  // Translate Phase 1 intake fields if needed
  intake = translatePhase1Intake(intake)
...
${roleSegmentBlock}
${readinessBlock}
${intake._phase1_intake ? `
PHASE 1 CONTEXT (EARLY-STAGE STARTUP):
This is an early-stage company using CompFrame's simplified intake. Design recommendations should be:
- Simple enough for a founder or first-time VP Sales to understand and administer
- Deployable immediately without a RevOps team or dedicated comp admin
- Conservative on complexity: when in doubt, simpler is better
${intake._ote_range_hint ? '- Buyer\'s Target OTE Range: ' + intake._ote_range_hint : '- No specific OTE target provided; use market benchmarks'}
${intake._clawback_preference ? '- Clawback Preference: ' + intake._clawback_preference : ''}
${intake._miss_handling ? '- Below-Quota Preference: ' + intake._miss_handling : ''}
` : ''}
`
