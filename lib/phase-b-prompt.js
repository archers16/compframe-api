...
export function buildPhaseBUserPrompt(intakeContext, phaseAOutput, metadata) {
  const roleArchitecture = JSON.stringify(phaseAOutput.role_architecture)
...
CRITICAL: You are generating ${pc} roles. Token efficiency is essential to avoid truncation.
...
- OMIT ramp_schedule array. Just provide ramp_months and draw_type; client generates the schedule.
- OMIT fields that are null or false entirely (skip them, don't include "field": null).
- OMIT spif_eligible (default true) and payout_lag_days (default 15) unless non-default.
- OMIT earnings_at_80pct, earnings_at_100pct, earnings_at_120pct, earnings_at_150pct. Client calculates from base_salary, target_variable, and accelerator_tiers.
- OMIT commission_rate and commission_rate_display unless the role uses rate-based comp.
- OMIT decelerator_tiers if has_decelerator is false.
- OMIT clawback_window_days and clawback_type if has_clawback is false.
- Use minimal JSON whitespace. No unnecessary nesting.
With ${pc} roles, every omitted field saves significant tokens. Prioritize completeness of all ${pc} roles over field density per role.`
    : pc > 5
    ? `\n\nCOMPACT OUTPUT MODE (${pc} plans detected):
...
- OMIT spif_eligible (default true) and payout_lag_days (default 15) unless non-default.
This is critical: with ${pc} plans, be token-efficient or output will be truncated.`
    : ''
