...
export function buildPhaseAUserPrompt(intakeContext, metadata) {
  ...
  const compactNote = (metadata.planCount > 3 || metadata.isMultiSegment)
    ? `\n\nCOMPACT MODE (${metadata.planCount} plans): Keep direction fields to 1 sentence each. Use "Same approach as [role_key] except: [differences]" for roles that share similar strategies. Focus on what differentiates each role, not shared defaults. Keep key_design_decisions to 3-4 max. Keep anti_patterns_detected to top 3. Keep comp_philosophy to 1 paragraph.`
    : ''
